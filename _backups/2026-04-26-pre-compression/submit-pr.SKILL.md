...
Collapse all approval gates into single confirmation. Show: files to stage, proposed commit message, PR title. Execute all steps (add + commit + push + create/update PR) without pausing. Stop only on error.

**Explicit approval mode:** If user says "step by step" or "confirm each step", switch to multi-gate workflow below.

Use GitHub MCP tools for all GitHub operations. Fall back to `gh` CLI only if MCP unavailable.

---

## Phase 1 — Pre-flight checks

### 1a — Confirm Jira ticket

PR's `## JIRA TASK:` must link to **ENG-\*** ticket — never IMPL directly.

- **ENG-\*** ticket in hand: use it.
- **IMPL-\*** ticket: fetch via Atlassian MCP, check `issuelinks` for linked ENG-\*.
  - Found: use that ENG ticket.
  - Not found: transition IMPL to "Code Fix Needed" first (transition id `121` on `eightfoldai.atlassian.net`) — auto-creates ENG ticket.

Note test environment from Jira description (sandbox URL, credentials) for TEST PLAN.
...
Flag if branch uses IMPL number or wrong format.
...
# JS/TS — husky runs on commit, but catch early:
...
Stage **only ticket-relevant files** — never `git add .`. Leave unrelated changes unstaged.

Show user: (1) files to stage, (2) proposed commit message, (3) that add+commit+push runs in one go. **Wait for approval, then run all three without pausing.**
...
Pre-push hooks run automatically. Fix any hook failures before re-attempting.

---

## Phase 3 — Create or update the PR

Check for existing PR via GitHub MCP `list_pull_requests` filtered by `head:<branch-name>`.
- **PR exists** → `update_pull_request`
- **No PR** → `create_pull_request`

**Auto-detect repo template** via `git remote get-url origin`:
- **wipdp** → prose format: Summary (2-4 sentences), JIRA TASK link, TEST PLAN (runnable bash block). Write body to temp file, use `--body-file`.
- **vscode** or other → read `.github/PULL_REQUEST_TEMPLATE.md` and fill checklist below.

**Draft mode:** If user says "keep in draft" or "draft mode", pass `--draft` flag.

**CRITICAL:** Always read `.github/PULL_REQUEST_TEMPLATE.md` from local filesystem — never reconstruct from memory or MCP response. MCP encodes special characters as HTML entities which breaks CI. **ALWAYS** write PR body to temp file and pass `--body-file`. Never pass body as inline string.

Fill all sections, then self-validate before submitting (Phase 4).
...
1. **Never delete any checklist item** — keep all `[ ]` lines verbatim from template
2. **Only add `x`**: `[ ]` → `[x]`, never remove items
3. **Never modify item text** — copy exactly
4. **URLs byte-for-byte identical** — especially "deploying all services" Atlassian URL
5. **Fill all `_____` blanks** — replace with value or `N/A`, never leave blank
...
- **Gate Control** — gated instructions, or one "not gated because" reason
...
- **PR Description for customer release** — required or not
...
**Auto-added items** (do NOT add manually — CI bot adds when triggered):
...
**Frontend / customer-facing (manual):**
```markdown
## TEST PLAN:
...
```
Use credentials/URLs from Jira ticket.

---

## Phase 4 — Self-validate before submitting

Scan PR body before calling `create_pull_request` or `update_pull_request`:

1. **No HTML entities** — search for `&amp;`, `&#39;`, `&quot;`. Decode if found. MCP encodes these; CI rejects them.
2. **No unfilled blanks** — search for `_____`. Every one replaced.
3. **Mandatory sections covered** — every section has at least one `[x]`.
