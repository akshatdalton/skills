...
On entry, MUST invoke `python3 ~/.claude/scripts/progress_fm.py get <TICKET_ID>` first. Surface one-line `↳ loaded ...` or `↳ no context yet`.

After PR is created/updated, MUST invoke `python3 ~/.claude/scripts/progress_fm.py append-section <TICKET_ID> --section "Decisions" --line "PR #<N> created at <url>, base=<branch>"` and surface `↳ saved to progress.md: ...`. Also bubble up to project layer if this PR is part of a stack: appending to `~/opensource/vault/wiki/projects/<repo>/learnings.md` under `## Initiative: <slug>` (one-line).
...
- **Standalone invocation** (user types `/submit-pr` directly): prompt:
  ...
  On user override `skip` → log to `~/opensource/vault/wiki/projects/<repo>/log.md` (per-vault-v1; `<repo>` resolved from ticket): "tests skipped — user override".
...
- **Exists** → **update-mode**: `gh pr edit <N> --body-file <body.md>` for description changes; `git push` for code commits. Update progress.md frontmatter `last-touched` and set `ci_state=pending` via `progress_fm.py set`.
...
Before completing:
1. `python3 ~/.claude/scripts/progress_fm.py append-section <TICKET_ID> --section "Decisions" --line "<PR url + CI status>"` (existing).
2. **progress.md** — register PR on the task and advance state:

```bash
REPO=$(git remote get-url origin | sed -E 's#.*/([^/.]+)(\.git)?$#\1#')
python3 ~/.claude/scripts/progress_fm.py set <TICKET_ID> \
  --field project="$REPO" --field branch="$(git branch --show-current)" --field pr=<PR_NUMBER> --field state=in-review
```
...
### Reads (Memory)
- `~/opensource/vault/wiki/projects/<repo>/progress/<TICKET_ID>/progress.md` — branch, project, ticket info, Files of interest, current pr (for update-mode) via `progress_fm.py get`

### Writes (Memory)
- `~/opensource/vault/wiki/projects/<repo>/progress/<TICKET_ID>/progress.md` — state=in-review (create-mode), pr=#XXXX, last-touched, ci_state=pending (update-mode) via `progress_fm.py set`
- `~/opensource/vault/wiki/projects/<repo>/learnings.md` — append PR-creation/update line under `## Initiative: <slug>` if part of a stack
- `~/opensource/vault/wiki/projects/<repo>/log.md` — per-vault-v1: per-project log; "submitted <repo>#XXXX" or "pushed update to <repo>#XXXX"
